...
{
    mpsm.emitRegisterDBusEvent(
        mpsm.bus, std::make_shared<sdbusplus::asio::object_server>(mpsm.bus));
...
{
    EXPECT_THROW(mpsm.emitRegisterDBusEvent(
                     mpsm.bus, std::make_shared<sdbusplus::asio::object_server>(
                                   mpsm.bus)),
                 sdbusplus::exception::SdBusError);
}
