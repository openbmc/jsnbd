  ...
  private:
    bool loadConfiguration(const std::string& file) noexcept
    {
        try
        {
            ...
            nlohmann::json data =
                nlohmann::json::parse(*configData, nullptr, false);
                              ...
                              const size_t standardMountPoints)
    {
        ...
        {
            std::optional<MountPoint> mp =
                createMountPoint(mountPointCounter, Mode::proxy);
            if (!mp)
            {
                return false;
            }
            std::string slotStr =
                std::format("{}_{}", slotPrefix, mountPointCounter);
        ...
        {
            std::optional<MountPoint> mp =
                createMountPoint(mountPointCounter, Mode::standard);
            if (!mp)
            {
                return false;
            }
    ...
    static std::optional<MountPoint>
        createMountPoint(const size_t index, const Mode mode)
    {
        MountPoint mp;
        ...
        std::string unixSocketStr =
            std::format("/run/virtual-media/nbd{}.sock", index);
