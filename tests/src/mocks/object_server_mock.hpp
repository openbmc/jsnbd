...
struct register_property_mock
{
    ...
    MOCK_METHOD(bool, register_property,
                (const std::string&, T, std::function<int(const T&, T&)>&&,
    ...
    public MockType<set_property_mock<PropertyTypes>>...
{
  public:
    ...
    MOCK_METHOD(bool, register_method, (const std::string&, DefaultMethod&&));
    MOCK_METHOD(bool, register_method,
                (const std::string&, LegacyMountMethod&&));
    ...
    void returnByDefault(const bool expectedReturn)
    {
        ...
        ON_CALL(*this, register_method(_, An<DefaultMethod&&>()))
            .WillByDefault(Return(expectedReturn));
        ON_CALL(*this, register_method(_, An<LegacyMountMethod&&>()))
            .WillByDefault(Return(expectedReturn));
...
class object_server_mock
{
  public:
      ...
      public:
            ...
            add_interface(const std::string& path, const std::string& name)
        {
            auto it = std::find_if(
                ...
                return (interface->path == path) && (interface->name == name);
            });
    ...
    // explicit operator sdbusplus::asio::object_server() const
    // {}
...
using dbus_interface_mock = MockType<
    impl::dbus_interface_mock<bool, uint8_t, uint16_t, int32_t, std::string>>;
