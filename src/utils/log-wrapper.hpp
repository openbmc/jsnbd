...
class LoggerWrapper
{
  public:
    ...
    template <typename T>
    LoggerWrapper& operator<<([[maybe_unused]] const T& value)
    {
        if constexpr (L <= kCurrentLogLevel)
        {
            // Ignore possible implicit conversions from array to pointer.
            // Target logger implementation is going to be reworked in a manner
            // resembling the one from bmcweb (using std::format instead of
            // string streams)
