...
class DeviceMonitor
{
  public:
        ...
        boost::asio::spawn(ioc, [this,
            ...
            while (true)
            {
                ...
                if (device)
                {
                    ...
                    if (sizeStr.empty())
                    {
                        LOGGER_ERROR << "[DeviceMonitor]: Received NULL size.";
                    ...
                    {
                        LOGGER_ERROR << "[DeviceMonitor]: Could not convert "
                                        "size "
                                        "to integer: "
                                     << e.what();
                    ...
                    {
                        LOGGER_INFO << "[DeviceMonitor]: " << nbdDevice.toPath()
                                    << " inserted.";
    ...
    {
        LOGGER_INFO << "[DeviceMonitor]: watch on " << device.toPath();
  ...
  public:
    ...
    bool spawn(const std::vector<std::string>& args, ExitCb&& onExit)
    {
        ...
        boost::asio::spawn(ioc, [this, self = shared_from_this(),
            ...
            LOGGER_INFO << "[Process]: running: " << child.running()
                        << " EC: " << child.exitCode()
    ...
    void stop(OnTerminateCb&& onTerminate)
    {
        boost::asio::spawn(ioc, [this, self = shared_from_this(),
            ...
            if (child.running())
            {
                LOGGER_INFO << "[Process] Terminate if process doesnt "
                               "want to exit nicely";
...
struct UsbGadget
{
  ...
  public:
    ...
    static int32_t configure(const std::string& name,
                             ...
                             StateChange change, const bool rw = false)
    {
        LOGGER_INFO << "[App]: Configure USB Gadget (name=" << name
                    << ", path=" << path
                    << ", State=" << static_cast<uint32_t>(change) << ")";
        bool success = true;

        if (change == StateChange::unknown)
        {
            LOGGER_CRITICAL << "[App]: Change to unknown state is not possible";
            ...
            try
            {
                     ...
                     std::filesystem::directory_iterator(Dirs::busPrefix()))
                {
                        ...
                        !Printer::exists(port.path() / "gadget/suspended"))
                    {
                        ...
                        LOGGER_DEBUG << "Use port : " << port.path().filename();
    ...
    static std::optional<std::string> getStats(const std::string& name)
    {
        ...
        if (!ifs.is_open())
        {
            LOGGER_ERROR << name << "Failed to open " << statsPath;
            return {};
        }
...
class UdevGadget
{
  ...
  public:
    ...
    static bool forceUdevChange()
    {
        ...
        {
            LOGGER_ERROR << "[App]: UdevGadget: " << e.what();
            return false;
        }
