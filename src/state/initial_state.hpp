  ...
  private:
    ...
    void cleanUpMountPoint()
    {
        ...
        if (std::filesystem::exists(localFile))
        {
            ...
            {
                LOGGER_INFO << "Cleanup directory " << localFile;
                std::error_code ec;
                if (!std::filesystem::remove(localFile, ec))
                {
                    LOGGER_ERROR
                        << ec << "Cleanup failed - unable to remove directory "
                        << localFile;
                }
    ...
    void addMountPointInterface(const RegisterDbusEvent& event)
    {
        ...
        iface->register_property(
            ...
            return static_cast<int>(config.remainingInactivityTimeout.count());
        });
        iface->initialize();
    }
        ...
        if (isLegacy)
        {
            ...
            iface->register_method(
                ...
                if (std::holds_alternative<unix_fd>(fd))
                {
                    ...
                    auto nullCount = std::count(buf.begin(), buf.begin() + size,
                                                '\0');
                    if (nullCount != 2)
                    {
                        throw sdbusplus::exception::SdBusError(
